...
# # Load cleaned data for each state
# state_files = {
...
#     return pd.DataFrame({'State': list(state_layoffs.keys()), 'Layoffs': list(state_layoffs.values())})

# # Prepare company-level data
# def get_company_layoffs():
#     company_layoffs = {}
#     for state, path in state_files.items():
#         try:
#             df = pd.read_csv(path)
...
#         except Exception as e:
#             pass
#     return pd.DataFrame(company_layoffs.items(), columns=['Company', 'Layoffs'])

# app.layout = dbc.Container([
#     dbc.Row([
#         dbc.Col(html.H1('Tech Layoffs Treemaps (2025)', className='text-center mb-4'), width=12)
#     ]),
#     dbc.Tabs([
#         dbc.Tab(
#             dcc.Graph(id='state-treemap',
#                 figure=px.treemap(get_state_layoffs(), path=['State'], values='Layoffs',
...
#         dbc.Tab(
#             dcc.Graph(id='company-treemap',
#                 figure=px.treemap(get_company_layoffs(), path=['Company'], values='Layoffs',
#                                   title='Tech Layoffs by Company (2025)',
#                                   color='Layoffs', color_continuous_scale=px.colors.sequential.Greens)
